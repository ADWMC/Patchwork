...
This documentation was distilled from the engineering practices and
maintenance records developed in the `helm-d` project, together with the
following reading supplied as a design reference:
...
The separation is deliberate: this repository is a clean design and
documentation base for a future code-maintenance agent, not a fork of the
security-analysis product.

## Status

The documents describe intended engineering behavior. Any future implementation
must introduce its own architecture, tests, installation contract, and release
verification before those behaviors can be claimed as delivered.
